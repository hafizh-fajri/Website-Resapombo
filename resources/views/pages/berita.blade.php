        ...
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-12 relative">
            ...
            <div class="relative flex-1 w-full overflow-hidden transition-all duration-300 ease-in-out max-h-[42px]" id="category-container">
                <div class="flex flex-wrap items-center gap-3 pr-12" id="category-list">
                    ...
                    <!-- Menggunakan tag <a> agar berfungsi sebagai link filter -->
            ...
            <!-- Input Pencarian -->
            <!-- Diubah menjadi Form agar bisa di-submit -->
            <form action="{{ url()->current() }}" method="GET" class="relative w-full md:w-80 shrink-0">
                <!-- Mempertahankan filter kategori saat melakukan pencarian -->
                @if($kategoriId)
                        ...
                        @forelse($beritaTerbaru as $highlight)
                            <div class="swiper-slide bg-white grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-10 p-5 md:p-8">
                                <div class="md:col-span-6 lg:col-span-7 rounded-2xl overflow-hidden aspect-[4/3] md:aspect-video relative bg-slate-100">
                                    <img src="{{ asset($highlight->gambar) }}" alt="{{ $highlight->judul }}" class="w-full h-full object-cover">
                                ...
                                <div class="md:col-span-6 lg:col-span-5 flex flex-col justify-center py-2">
                                    <div>
                                        ...
                                        <h3 class="text-2xl md:text-3xl font-bold text-slate-950 mb-4 leading-tight">{{ $highlight->judul }}</h3>
                                        <p class="text-slate-600 text-base leading-relaxed mb-8 line-clamp-3 md:line-clamp-4">
                                            {{ Str::limit(strip_tags($highlight->isi), 180) }}
                                        </p>
                                    </div>
                                    <div>
                                        <a href="{{ url('berita/' . $highlight->slug) }}" class="inline-flex items-center gap-2 bg-[#0F5132] text-white px-6 py-3 rounded-xl text-sm font-semibold hover:bg-[#146C43] transition-colors shadow-sm">
                                            Baca Selengkapnya
        ...
        <div>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                ...
                @forelse($berita as $item)
                    <article class="bg-white rounded-3xl border border-[#E9ECEF] p-5 shadow-sm hover:shadow-lg transition-shadow duration-300 flex flex-col h-full">
                        <div class="rounded-2xl overflow-hidden aspect-[4/3] mb-5 relative bg-slate-100 shrink-0">
                            <img src="{{ asset($item->gambar) }}" alt="{{ $item->judul }}" class="w-full h-full object-cover hover:scale-105 transition-transform duration-500">
                        ...
                        <div class="flex items-center justify-between gap-3 text-sm border-t border-[#E9ECEF] pt-4 mt-auto">
                            ...
                            <a href="{{ url('berita/' . $item->slug) }}" class="text-[#286B39] font-bold flex items-center gap-1 hover:gap-2 transition-all">
                                Baca Selengkapnya
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                            </a>
                        </div>
